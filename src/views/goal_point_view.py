...
class GoalPointView(ttkb.Frame):
    def __init__(self, parent, preview_point):
        ...
        self.x_input = ttkb.Entry(first_row, textvariable=self.x, width=5)
        self.y_input = ttkb.Entry(first_row, textvariable=self.y,width=5)
        self.z_input = ttkb.Entry(first_row, textvariable=self.z,width=5)
        ...
        self.x_slider = ttkb.Scale(x_frame, from_=-1, to=1, length=100, command=lambda value: self.update_variable(self.x, value, preview_point))
        self.y_slider = ttkb.Scale(y_frame, from_=-1, to=1, length=100, command=lambda value: self.update_variable(self.y, value, preview_point))
        self.z_slider = ttkb.Scale(z_frame, from_=-1, to=1, length=100, command=lambda value: self.update_variable(self.z, value, preview_point))
        ...
        self.points_table = ttkb.tableview.Tableview(self, coldata=["Point_ID", "Coordinates"], rowdata=[], height=6)
